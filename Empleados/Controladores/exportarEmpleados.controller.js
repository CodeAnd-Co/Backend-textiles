...
/**
 * Controlador para exportar empleados y retornar CSV como string en JSON.
 * 
 *
 * 
 * @async
...
exports.exportarEmpleados = async (req, res) => {
  try {
    const empleados = await repositorio.obtenerEmpleadosExportacion();
    ...
    return res.status(MENSAJES_EMPLEADOS.LISTA_EMPLEADOS_EXPORTADA.codigo).json({
      ...
      csv // string con contenido CSV para que el frontend lo descargue
    });
  } catch (error) {
