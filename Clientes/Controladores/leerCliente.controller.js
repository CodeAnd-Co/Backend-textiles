...
exports.leerCliente = async (req, res) => {
  ...
  try {
    ...
    let imagenCliente = '/placeholder.png';
    try {
      const imagenes = await obtenerImagenFolder(req, `clientes/${idCliente}/`);
      if (imagenes && imagenes.length > 0) {
        imagenCliente = imagenes[0].urlImagen;
      }
    } catch (errImg) {
      console.warn('Error al obtener imagen del cliente, se usará placeholder:', errImg);
    }
