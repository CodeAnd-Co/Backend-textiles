...
exports.obtenerCuota = async () => {
  ...
  try {
    ...
    const [resultadoReseteo] = await conexion.query(QUERY.RESETEAR_LIMITES);

    if (resultadoReseteo.changedRows === 0) {
      await conexion.rollback();
      return {
        error: 'Ninguna columna se actualizó. No se actualizará la fecha.',
      };
    }

    await conexion.query(QUERY.ACTUALIZAR_FECHAS);

    await conexion.commit();

    return { exito: 'Actualización exitosa' };
  } catch (error) {
    await conexion.rollback();
